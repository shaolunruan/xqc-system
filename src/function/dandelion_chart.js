...
function dandelion_chart(state_vector_, states, svg_element, size_arr_, position_arr_, theta_){
    ...
    if(typeof state_vector_ !== 'object' || state_vector_.length !== 4){
    ...
    // 定义变量
    let state_vector = state_vector_ || 'parameter error'
    let all_possible_states = states || 'parameter error'
    let svg = svg_element || 'parameter error'
    ...
    const view_padding_left = 20, view_padding_top = 20
    ...
    let view = svg.append('g') // 外面套的边框是 黑色的border
        .attr('class', function(){
    ...
    state_g0.append('circle')
        ...
        .style("stroke", (d,i)=>colorMap_circle_border[i])
        .style("stroke-width", 1)
        .style("fill", (d,i)=>colorMap_circle_border[i])
    ...
    let points = state_g.append('circle')
        ...
        // .style("stroke", "#7a0099")
        // .style("stroke-width", 1)
        .style("fill", (d,i)=>colorMap_point[i])
